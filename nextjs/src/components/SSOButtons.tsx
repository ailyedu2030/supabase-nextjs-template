...
function getEnabledProviders(): Provider[] {
  ...
  return providersStr
    .split(',')
    .filter((provider): provider is Provider => provider.trim().toLowerCase() in PROVIDER_CONFIGS);
}
          ...
          return (
            ...
            >
              <div className="absolute left-6">
                <div className="flex h-5 w-5 items-center justify-center">{config.icon}</div>
              </div>
              <span className="mx-auto text-sm font-semibold">Continue with {config.name}</span>
            </button>
          );
        })}
      </div>
